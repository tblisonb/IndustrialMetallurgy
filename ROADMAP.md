...
## Part 22 — Autoclave: real acid/alkaline/cyanide leaching, and a new metal-yield chain — **DONE**

A new late-game machine and processing chain, based on real hydrometallurgy (acid leaching): a
third yield tier for crushed ore, above the plain furnace and the Arc Furnace's 50% forge bonus.
Grew out of an idea the user flagged in Part 21 (open item 7) and a design conversation
researching real leaching processes (2026-09-03).

### Why one machine, not two
The original pitch had a mid-game "Leaching Tank" and an end-game "Autoclave." The user pointed
out (2026-09-03) that a real autoclave is just a pressure vessel -- it can run a mild,
atmospheric-pressure leach exactly as well as a genuine HPAL-grade one, just at different
settings. So there's only one machine, the Autoclave, gated late-game by its crafting cost
(Controller Board/Refractory Composite/Heat Sink/Tungsten Steel Ingot/Battery Bank), not by a
second machine tier.

### Lixiviants: acid, alkaline, and cyanide, matched to real chemistry
Not every ore leaches in the same reagent. Nickel/zinc/cobalt/manganese/titanium ore (garnierite,
sphalerite, cobaltite, pyrolusite, rutile) leach in sulfuric acid -- the mod's existing Sulfuric
Acid Bottle. Bauxite needs a base (new Sodium Hydroxide Bottle -- the real Bayer-process reagent
used to pull alumina out of bauxite). Argentite needs cyanide (new Sodium Cyanide Bottle -- the
user was explicit (2026-09-03) that chemical accuracy mattered more than the optics of a
"cyanide" item name). Chromite and scheelite were left out of this pass -- their real leaching
routes are less clean fits and weren't asked for.

### The chain: Autoclave -> Chemical Reactor -> smelt
Autoclave: crushed ore + lixiviant bottle + FE -> a pregnant leach solution bottle (7 new bottle
items, one per metal, named for their real intermediate compounds -- nickel/zinc/cobalt/
manganese_sulfate_bottle, titanyl_sulfate_bottle, sodium_aluminate_bottle, silver_cyanide_bottle).
A solution alone is inert -- the Chemical Reactor precipitates it (add Calcium Oxide) into a metal
concentrate, reusing the Reactor's existing generic 3-input/2-output recipe machinery with *zero*
Java changes, just new recipe JSON. Concentrate then smelts (plain vanilla `minecraft:smelting`,
so both a furnace and the Electric Furnace accept it) into ingots at 2x the crushed-ore baseline --
better than the Arc Furnace's 1.5x, which is the entire point of the extra steps, reagent cost,
and power draw.

### Calcium Sulfate, a real byproduct
Precipitating a sulfate solution (5 of the 7 metals) with Calcium Oxide really does yield gypsum
(CaSO4) alongside the concentrate -- added as a new item, correctly *not* produced by the two
non-sulfate recipes (alumina/silver), since there's no sulfate ion in those solutions. Not
consumed by anything yet -- the user's own idea (2026-09-03) is decorative blocks, or a future
Cultivator fertilizer input; left open.

### Bottles, not a fluid system
Leach solutions are ordinary items (`X_bottle`), the same convention `sulfuric_acid_bottle`/
`petroleum_bottle`/etc. already use -- not NeoForge fluid capability. Discussed explicitly
(2026-09-03): a real fluid system (tanks, `IFluidHandler`) would be a good future upgrade,
comparable in scope to the Part 20 transfer-API migration, but was deliberately not built for this
Part -- the bottle convention ships the whole chain today, and the recipe *shapes* would carry
over largely unchanged if fluids arrive later. See Part 5 open item 2.

### New machine, following the Chemical Reactor's pattern
`AutoclaveBlock`/`AutoclaveBlockEntity`/`AutoclaveContainer`/`AutoclaveScreen`/`AutoclaveRecipe`+
`AutoclaveRecipeInput`, registered the same way every other machine is (`ModTileEntityTypes`/
`ModContainerTypes`/`ModRecipes`/`RegistryHandler`/`IndustrialMetallurgyClient`/JEI plugin+
category). 3 input slots (matches the Reactor's multi-ingredient matching), 1 output slot,
250,000 FE capacity and a 100-tick process time (vs. the Reactor's 100,000 FE/50 ticks) to earn
its late-game placement through genuine cost rather than an arbitrary gate.

### Textures are procedural placeholders
Bottle/chunk icons for all 17 new items were generated by hue-shifting the existing
`sulfuric_acid_bottle`/`calcium_oxide` icons per-item (same "procedural placeholder, meant to be
hand-touched-up later" approach as the Part 9 ore textures) -- not hand-drawn. The Autoclave's
block-front and GUI container textures are new, hand-coded pixel art (steel/riveted
pressure-vessel theme, distinguishing it from the Reactor's look), not ported from anything.

### What's still unverified
Compiles clean, builds clean, and a dedicated server was watched loading with 1885 recipes (up
from Part 21's 1861 -- the expected +24: 7 leach + 7 precipitation + 2 lixiviant-production + 7
smelt + 1 crafting recipe) and no errors. Not yet confirmed with a live client: the Autoclave's
GUI layout/textures actually looking right on screen, that the full 3-step chain (leach ->
precipitate -> smelt) produces the expected 2x yield in practice, or that the new item icons read
clearly at inventory size.
...
7. **Acid wash machine — resolved, see Part 22.** Built as the Autoclave: crushed ore + a lixiviant
   bottle (sulfuric acid, or the new Sodium Hydroxide/Sodium Cyanide bottles, matched to each ore's
   real leaching chemistry) + FE, feeding into the Chemical Reactor for precipitation and then a
   normal smelt, for a 2x yield versus a direct furnace smelt. Late-game gated by crafting cost, not
   a separate machine tier. Chromite/scheelite leaching, and a real in-mod fluid system to replace
   the bottle-item convention, are still open (see item 2 above).
